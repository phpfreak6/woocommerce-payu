...
class Payu_API {
    ...
    public static function create_customer( $user_id, $customer_data ) {
        ...
        $customer = Payu_API::post_data( $customer_data, 'customers' );

        $key = Payu_API::find_card_index( $customer->sources->data, 'id', $customer->default_source );
        ...
        if ( $key > -1 ) {
            ...
            Payu_DB::update_customer( $user_id, $customerArray );
    ...
    public static function get_customer( $customer_id ) {
		...
		return Payu_API::get_data( 'customers/' . $customer_id );
    }
    ...
    public static function update_customer( $customer_id, $customer_data ) {
        ...
        return Payu_API::post_data( $customer_data, 'customers/' . $customer_id );
		
    }

    /**
     * Delete card from payu servers
     *
     * @access      public
     * @param       int $user_id
     * @param       int $position
    ...
    public static function delete_card( $user_id, $position = 0 ) {
	   ...
	   $user_meta = get_user_meta( $user_id, $this->db_location, true );

        Payu_DB::delete_customer( $user_id, array( 'card' => $user_meta['cards'][ $position ]['id'] ) );

        return Payu_API::delete_data( 'customers/' . $user_meta['customer_id'] . '/cards/' . $user_meta['cards'][ $position ]['id'] );
    }

    /**
     * Create charge on payu servers
     *
     * @access      public
    ...
    public static function create_charge( $charge_data ) {
        return Payu_API::post_data( $charge_data );
    }

    /**
     * Capture charge on payu servers
     *
     * @access      public
    ...
    public static function capture_charge( $transaction_id, $charge_data ) {
        return Payu_API::post_data( $charge_data, 'charges/' . $transaction_id . '/capture' );
    }

    /**
     * Create refund on payu servers
     *
     * @access      public
     * @param       string $transaction_id
    ...
    public static function create_refund( $transaction_id, $refund_data ) {
        return Payu_API::post_data( $refund_data, 'charges/' . $transaction_id . '/refunds' );
    }

    /**
     * Parse payu's response after interacting with the API
     *
     * @access      public
