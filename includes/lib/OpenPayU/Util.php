...
class OpenPayU_Util
{
  ...
  0  *
	...
	public static function buildarParams($order , $secretKey = 'SECRET_KEY')
    {
       ksort($order);
			...
			$order["ORDER_HASH"] = hash_hmac("md5", $hashString, $secretKey);
    ...
    public static function convertArrayToHtmlForm($array, $namespace = "", &$outputFields)
    {
        ...
        foreach ($array as $key => $value) {
            ...
            if (is_array($value)) {
                ...
                $htmlOutput .= sprintf("<input type=\"hidden\" name=\"%s\" value=\2%s\" />\n", $key, $value);
